...
#define DESC_S_CODE	1       // 代码段和数据段属于存储段，tss和各种门描述符属于系统段
...
#define TI_GDT 0   // GDT描述符
#define TI_LDT 1   // IDT描述符

#define SELECTOR_K_CODE	   ((1 << 3) + (TI_GDT << 2) + RPL0) // 内核代码段选择子
#define SELECTOR_K_DATA	   ((2 << 3) + (TI_GDT << 2) + RPL0) // 内核数据段选择子
#define SELECTOR_K_STACK   ((2 << 3) + (TI_GDT << 2) + RPL0) // 内核栈段选择子
#define SELECTOR_K_GS	   ((3 << 3) + (TI_GDT << 2) + RPL0) // 显存段选择子
